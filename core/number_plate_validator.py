...
def _get_city_name(plate_number: str) -> str:
    """
    ...
    try:
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            if type(row[1]) is int:
                if not '،' in row[3]:
                    if row[1] and row[3] and num == int(row[1]) and letter == row[3] :
                        wb.close()
                        return {
                            'city': re.sub(r'\xa0', '', row[0]),
                            'town': re.sub(r'\xa0', '', row[2])
                        }
                else:
                    row3 = [s.strip() for s in row[3].split('،')]
                    if row[1] and row[3] and num == int(row[1]) and letter in row3:
                        wb.close()
                        return {
                            'city': re.sub(r'\xa0', '', row[0]),
                            'town': re.sub(r'\xa0', '', row[2])
                ...
                row1 = np.array(row[1].split('،')).astype('int32')
                if not '،' in row[3]:
                    if row[1] and row[3] and num in row1 and letter == row[3] :
                        wb.close()
                        return {
                            'city': re.sub(r'\xa0', '', row[0]),
                            'town': re.sub(r'\xa0', '', row[2])
                    ...
                    row3 = [s.strip() for s in row[3].split('،')]
                    if row[1] and row[3] and num in row1 and letter in row[3].split('،'):
                        wb.close()
                        return {
                            'city': re.sub(r'\xa0', '', row[0]),
                            'town': re.sub(r'\xa0', '', row[2])
...
def plate_filter(plate_number: str) -> dict:
    ...
    """
    cleaned = ''.join(re.findall(r"[آابپتثجچحخدذرزژسشصضطظعغفقکگلمنوهیئ|0-9]+", plate_number))
    if len(cleaned) != 8:
        raise ValueError("plate number length is incorrect!")
    if not bool(re.search(r"[0-9]{2}[آابپتثجچحخدذرزژسشصضطظعغفقکگلمنوهیئ]{1}[0-9]{5}", cleaned)):
